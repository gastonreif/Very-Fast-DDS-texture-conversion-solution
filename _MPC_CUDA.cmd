...
del gnode0block1.txt
del gnode0block2.txt
del gnode1block3.txt
del gnode1block4.txt
del gnode2block5.txt
del gnode2block6.txt
del gnode3block7.txt
del gnode3block8.txt
del missing.inf
cls
...
ECHO    MASSIVE PARALLEL COMPUTING VERSION FOR CUDA (8 NODES)
ECHO      USES BOTH CPU AND GPU UNITS AT THE SAME TIME  
echo.%ch%
echo System informations:
echo CPU cores: %NUMBER_OF_PROCESSORS%
If /I %NUMBER_OF_PROCESSORS% LEQ 4 ECHO WARNING, MINIMUM CPU CORES ARE 8, YOUR SYSTEM CAN BE VERY OVERLOADED!
If /I %NUMBER_OF_PROCESSORS% LEQ 4 ECHO HIT SPACE TO CONTINUE OR CTRL+C TO QUIT AND RUN RATHER _CPU_Conversion.cmd
If /I %NUMBER_OF_PROCESSORS% LEQ 4 pause
...
IF %JPEGs% LEQ 32 goto :error
If /I %p% LEQ 32 goto :error
...
REM CREATING gnode0blocks
REM (ted preskoci Pulku Node A0 a vytvori Block 2 a pak Block 1 (NODE0,NODE1)
set /A X=0
set /A b=2
set /A X=%Z%/%b
echo Total files for GPU Node A0 block 1: %X%
REM ECHO breakpoint3
REM pause
for /f "skip=%X%" %%t IN (gnodeA0.txt) DO @echo %%t >> gnode0block2.txt
for /f %%d in ('Find /V /C "" ^< "gnode0block2.txt"') do set GNode0block2=%%d
echo Total files for GPU Node A0 block 2: %GNode0block2%
for /f "delims=" %%s in (gnodeA0.txt) do (
  find "%%s" <"gnode0block2.txt" || >>gnode0block1.txt echo %%s)
REM CREATING gnode1blocks
REM (ted preskoci Pulku Node A1 a vytvori Block 4 a pak Block 3 (NODE2,NODE3)
set /A Y=0
set /A b=2
set /A Y=%Z%/%b
echo Total files for GPU Node A1 block 3: %Y%
REM ECHO breakpoint4
REM pause
for /f "skip=%Y%" %%T IN (gnodeA1.txt) DO @echo %%T >> gnode1block4.txt
for /f %%D in ('Find /V /C "" ^< "gnode1block4.txt"') do set GNode1block4=%%D
echo Total files for GPU Node A1 block 4: %GNode1block4%
for /f "delims=" %%S in (gnodeA1.txt) do (
  find "%%S" <"gnode1block4.txt" || >>gnode1block3.txt echo %%S)
REM CREATING NODE B gnodes 0 and 1
set /A w=0
set /A b=2
set /A w=%o%/%b
echo Total files for GPU Node B subnode 0: %w%
REM ECHO breakpoint5
REM pause
REM (ted preskoci Pulku Node B a vytvori gnodeB1 a pak gnodeB0)
for /f "skip=%w%" %%u IN (gpunodeB.txt) DO @echo %%u >> gnodeB1.txt
for /f %%z in ('Find /V /C "" ^< "gnodeB1.txt"') do set gnodeB1=%%z
echo Total files for GPU Node B subnode 1: %gnodeB1%
for /f "delims=" %%h in (gpunodeB.txt) do (
  find "%%h" <"gnodeB1.txt" || >>gnodeB0.txt echo %%h)
REM CREATING gnode2blocks
REM (ted preskoci Pulku Node B0 a vytvori Block 6 a pak Block 5 (NODE4, NODE5)
set /A I=0
set /A b=2
set /A I=%w%/%b
echo Total files for GPU Node B0 block 5: %I%
REM ECHO breakpoint6
REM pause
for /f "skip=%I%" %%k IN (gnodeB0.txt) DO @echo %%k >> gnode2block6.txt
for /f %%l in ('Find /V /C "" ^< "gnode2block6.txt"') do set GNode2block6=%%l
echo Total files for GPU Node B0 block 6: %GNode2block6%
for /f "delims=" %%m in (gnodeB0.txt) do (
  find "%%m" <"gnode2block6.txt" || >>gnode2block5.txt echo %%m)
REM CREATING gnode3blocks
REM (ted preskoci Pulku Node B1 a vytvori Block 8 pak Block 7 (NODE6, NODE7)
set /A N=0
set /A b=2
set /A N=%w%/%b
echo Total files for GPU Node B1 block 7: %N%
REM ECHO breakpoint7
REM pause
for /f "skip=%N%" %%c IN (gnodeB1.txt) DO @echo %%c >> gnode3block8.txt
for /f %%F in ('Find /V /C "" ^< "gnode3block8.txt"') do set GNode3block8=%%F
echo Total files for GPU Node B1 block 8: %GNode3block8%
for /f "delims=" %%G in (gnodeB1.txt) do (
  find "%%G" <"gnode3block8.txt" || >>gnode3block7.txt echo %%G)
ECHO Conversion started by GPU (minimized in eight dedicated consoles) 
ECHO Conversion Started at %Time% >> timer.txt
start "NODE0 on GPU running" /MIN gnode0.cmd
start "NODE1 on GPU running" /MIN gnode1.cmd
start "NODE2 on GPU running" /MIN gnode2.cmd
start "NODE3 on GPU running" /MIN gnode3.cmd
start "NODE4 on GPU running" /MIN gnode4.cmd
start "NODE5 on GPU running" /MIN gnode5.cmd
start "NODE6 on GPU running" /MIN gnode6.cmd
start "NODE7 on GPU running" /MIN gnode7.cmd
...
for /F "delims=" %%m in ('findstr /I "NODE4" timer.txt') do set "GPUDONE4=%%m"
@find /c /i "NODE4" "timer.txt" > NUL
REM ERRORLEVEL0 IS SUCCESS
if %ERRORLEVEL% EQU 0 GOTO CheckGPU5
REM If we get here, the file is not found.
ECHO Waiting for GPU Node 4 Conversion end, please wait, it can take some time
TIMEOUT /T 1 >nul
GOTO CheckGPU4
:CheckGPU5
set GPUDONE5=
REM formula gathering times from timer for GPU5
for /F "delims=" %%n in ('findstr /I "NODE5" timer.txt') do set "GPUDONE5=%%n"
@find /c /i "NODE5" "timer.txt" > NUL
REM ERRORLEVEL0 IS SUCCESS
if %ERRORLEVEL% EQU 0 GOTO CheckGPU6
REM If we get here, the file is not found.
ECHO Waiting for GPU Node 5 Conversion end, please wait, it can take some time
TIMEOUT /T 1 >nul
GOTO CheckGPU5
:CheckGPU6
set GPUDONE6=
REM formula gathering times from timer for GPU6
for /F "delims=" %%O in ('findstr /I "NODE6" timer.txt') do set "GPUDONE6=%%O"
@find /c /i "NODE6" "timer.txt" > NUL
REM ERRORLEVEL0 IS SUCCESS
if %ERRORLEVEL% EQU 0 GOTO CheckGPU7
REM If we get here, the file is not found.
ECHO Waiting for GPU Node 6 Conversion end, please wait, it can take some time
TIMEOUT /T 1 >nul
GOTO CheckGPU6
:CheckGPU7
set GPUDONE7=
REM formula gathering times from timer for GPU7
for /F "delims=" %%Q in ('findstr /I "NODE7" timer.txt') do set "GPUDONE7=%%Q"
@find /c /i "NODE7" "timer.txt" > NUL
REM ERRORLEVEL0 IS SUCCESS
if %ERRORLEVEL% EQU 0 GOTO ALLISDONE
REM If we get here, the file is not found.
ECHO Waiting for GPU Node 7 Conversion end, please wait, it can take some time
TIMEOUT /T 1 >nul
GOTO CheckGPU7
:ALLISDONE
ECHO ----------------------------------------------------------------------------
...
IF %DONE% EQU 10 ECHO Conversion is DONE
REM IF %DONE% NEQ 10 GOTO :error
IF %DONE% LSS 10 GOTO :error
